...
expName = 'Staircase_Test'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/marie/Desktop/Psychopy Test/Staircase_Test/Staircase_Test.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    Welcome_Text = visual.TextStim(win=win, name='Welcome_Text',
        text='Left, red triangle is stable, Right, white triangle is comparison.\nLeft arrow key= make smaller\nRight arrow key= make bigger\n\nPress "Space" to move on.',
        font='Arial',
    ...
    Stable_Triangle = visual.ShapeStim(
        win=win, name='Stable_Triangle',units='cm', 
    ...
    Change_Triangle = visual.ShapeStim(
        win=win, name='Change_Triangle',units='cm', 
    ...
    conditions = data.importConditions('Staircasing Test.xlsx')
    ...
    for level, condition in trials:
        ...
        Trial = data.Routine(
            name='Trial',
            components=[key_resp, Stable_Triangle, Change_Triangle],
        )
        Trial.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from Ans_code
        # check data that it's actually marking CorrectAns according to >1
        if  map(lambda x: x>1, Change_Triangle.size):
            CorrectAns= 'left'
        else:
            CorrectAns= 'right'
        ...
        while continueRoutine:
            ...
            if key_resp.status == STARTED and not waitOnFlip:
                ...
                if len(_key_resp_allKeys):
                    ...
                    # was this correct?
                    if (key_resp.keys == str('(1,1)')) or (key_resp.keys == '(1,1)'):
                        key_resp.corr = 1
                    else:
                        key_resp.corr = 0
                    # a response ends the routine
            ...
            # *Stable_Triangle* updates
            
            # if Stable_Triangle is starting this frame...
            if Stable_Triangle.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Stable_Triangle.frameNStart = frameN  # exact frame index
                Stable_Triangle.tStart = t  # local t and not account for scr refresh
                Stable_Triangle.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Stable_Triangle, 'tStartRefresh')  # time at next scr refresh
                # update status
                Stable_Triangle.status = STARTED
                Stable_Triangle.setAutoDraw(True)
            
            # if Stable_Triangle is active this frame...
            if Stable_Triangle.status == STARTED:
                # update params
                pass
            
            # *Change_Triangle* updates
            
            # if Change_Triangle is starting this frame...
            if Change_Triangle.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
                Change_Triangle.frameNStart = frameN  # exact frame index
                Change_Triangle.tStart = t  # local t and not account for scr refresh
                Change_Triangle.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(Change_Triangle, 'tStartRefresh')  # time at next scr refresh
                # update status
                Change_Triangle.status = STARTED
                Change_Triangle.setAutoDraw(True)
            
            # if Change_Triangle is active this frame...
            if Change_Triangle.status == STARTED:
                # update params
                pass
